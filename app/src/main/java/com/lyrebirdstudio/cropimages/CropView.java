...
public class CropView extends View {
    ...
    int leftBorder = this.halfBorder;
    PointF leftDown;
    PointF leftUp;
    Matrix matrix;
    PointF midDown;
    ...
    float pointRadius = 25.0f;
    RectF rectDown;
    float rectH = 15.0f;
    RectF rectLeft;
    Paint rectPaint;
    float rectRadius = 6.0f;
    RectF rectRight;
    RectF rectUp;
    float rectW = 10.0f;
    int rightBorder;
    ...
    float sentinel = 70.0f;
    int topBorder = this.leftBorder;
    float width;
    ...
    PointF selectPoint(float x, float y) {
        if (((x - this.leftUp.x) * (x - this.leftUp.x)) + ((y - this.leftUp.y) * (y - this.leftUp.y)) < (this.pointRadius * this.pointRadius) * 2.0f) {
            return this.leftUp;
        }
        if (((x - this.leftDown.x) * (x - this.leftDown.x)) + ((y - this.leftDown.y) * (y - this.leftDown.y)) < (this.pointRadius * this.pointRadius) * 2.0f) {
            return this.leftDown;
        }
        if (((x - this.rightUp.x) * (x - this.rightUp.x)) + ((y - this.rightUp.y) * (y - this.rightUp.y)) < (this.pointRadius * this.pointRadius) * 2.0f) {
            return this.rightUp;
        }
        if (((x - this.rightDown.x) * (x - this.rightDown.x)) + ((y - this.rightDown.y) * (y - this.rightDown.y)) < (this.pointRadius * this.pointRadius) * 2.0f) {
    ...
    PointF selectMidPoint(float x, float y) {
        if (((x - this.midLeft.x) * (x - this.midLeft.x)) + ((y - this.midLeft.y) * (y - this.midLeft.y)) < (this.pointRadius * this.pointRadius) * 2.0f) {
            return this.midLeft;
        }
        if (((x - this.midUp.x) * (x - this.midUp.x)) + ((y - this.midUp.y) * (y - this.midUp.y)) < (this.pointRadius * this.pointRadius) * 2.0f) {
            return this.midUp;
        }
        if (((x - this.midRight.x) * (x - this.midRight.x)) + ((y - this.midRight.y) * (y - this.midRight.y)) < (this.pointRadius * this.pointRadius) * 2.0f) {
            return this.midRight;
        }
        if (((x - this.midDown.x) * (x - this.midDown.x)) + ((y - this.midDown.y) * (y - this.midDown.y)) < (this.pointRadius * this.pointRadius) * 2.0f) {
            return this.midDown;
        }
        return null;
    }
    ...
    public void setMode(int pMode) {
        ...
        if (this.mode == 1) {
           ...
           // this.multiplier = AdaptiveVideoTrackSelection.DEFAULT_BANDWIDTH_FRACTION;
        } else if (this.mode == 8) {
